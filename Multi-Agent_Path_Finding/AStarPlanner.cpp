    ...
    while (!open.empty()) {
        ...
        for (auto next_location : ins.get_adjacent_locations(curr->location)) {
            ...
            if (it == all_nodes.end()) {// the location has not been visited before
                ...
                Constraint next_same (agent_id, next_location, -1, next_time);
                Constraint next_move (agent_id, curr->location, next_location, next_time);
                for (auto check : constraints)
                {
                    if (check == next_same || check == next_move)
                    {
                        unacceptable = true;
                        break;
                    }
                ...
                {
                    open.push(next);
                    all_nodes[make_pair(next_location, next_time)] = next;
                }
                else
                {
                    delete next;
        ...
        auto next = new AStarNode(curr->location, curr->g + 1, curr->h, curr, curr->timestep + 1);
        bool unacceptable = false;
        Constraint next_same (agent_id, curr->location, -1, next->timestep);
        for (auto check : constraints)
        {
            if (check == next_same)
            {
        ...
        {
            open.push(next);
            all_nodes[make_pair(next->location, next->timestep)] = next;
        }
        else
        {
            delete next;
